        ...
        if (!checkUsr(usr, finalArr)) {
            ...
            waiting_users.forEach((ele) => {
                if (ele.username != usr.username && !checkEle(ele, finalArr)) {
                    distVl = match_config["s_weight"] * (usr.skill - ele.skill) + match_config["t_weight"] * (usr["waiting_time"] - ele["waiting_time"]) + match_config["l_weight"] * (usr["user_level"] - ele["user_level"]);
                    let tmpObj = {}
